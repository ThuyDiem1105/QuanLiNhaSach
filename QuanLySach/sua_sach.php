...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' && $book_id) {
  ...
  $stmt->bind_param('i',$book_id);
  $stmt->execute();
  $stmt->bind_result($category, $genre, $name, $author, $publisher, $publishDay, $language, $sellPrice, $currentQuantity);
  $stmt->fetch();
  $stmt->close();
}
        ...
        <h1>Cập nhật thông tin tài khoản nhân viên</h1>
            <form action="" method="post" class="editAccount-form" novalidate>
                <label class="label-form" for="id">Mã nhân viên</label>
                    ...
                    <button type="submit" name="submit_book">Cập nhật thông tin sách</button>
            </form>
    </body>
</html>
